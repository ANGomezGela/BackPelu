package eus.fpsanturtzilh.repository;

import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import eus.fpsanturtzilh.entity.Material_maileguak;

/**
 * {@link MaterialMaileguakRepository} interfazak {@link Material_maileguak} entitatearekin
 * lotutako datuak kudeatzeko funtzionalitatea eskaintzen du.
 * 
 * <p>Spring Data JPA-ren {@link JpaRepository} erabiltzen da, hau da, 
 * datu-baseko eragiketak (sartu, eguneratu, ezabatu, bilatu) egiteko funtzionalitatea
 * eskaintzen duen interfazea. Honek, {@link Material_maileguak} entitatearekin lotutako
 * operazioak modu automatikoan burutzen ditu.</p>
 * 
 * <p>Interfazearen galdera berezia:</p>
 * <ul>
 *   <li><strong>findByEzabatzeDataIsNull</strong>: Datu-basean ezabatutako
 *       mailegatutako materialak ez duen (aktiboak diren) materialen maileguen zerrenda lortzen du.</li>
 * </ul>
 * 
 * @author [Zure Izena]
 * @since [Data]
 */
@Repository
public interface MaterialMaileguakRepository extends JpaRepository<Material_maileguak, Long> {

    /**
     * Ezabatze data null duten material maileguak (aktiboak) lortzen dituen galdera.
     *
     * @return {@link List} Aktiboak diren material maileguak.
     */
    List<Material_maileguak> findByEzabatzeDataIsNull();
}
